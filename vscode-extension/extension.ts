...
class WebChatPasteServer {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        vscode.window.showErrorMessage(
          `Failed to start WebSocket server: ${errorMsg}`,
        );
        reject(error);
      }
    });
  ...
  broadcastMessage(message: object): number {
    let sentCount = 0;
    ...
    () => {
      ...
      if (clientCount === 0) {
        ...
        vscode.window.showWarningMessage(
          'No clients connected. Ensure browser extension is connected.',
        );
        return;
      }
      ...
      vscode.window.showInformationMessage(
        `Text sent to chat (${sentCount} clients).`,
      );
    },
  );
